...
class MetasService implements MetasServiceInterface {
    ...
    public function index($pesquisar, $perPage) {
        $registro = $this->repository->where(function($query) use($pesquisar){
            if($pesquisar){
                $query->where("tipo_meta","like","%".$pesquisar."%");
            }
        })->paginate($perPage);
        return $registro;
    }
    ...
    public function update(Request $request, string $id) {
        ...
        $metasCadastrado = $this->repository->find($id);

        DB::beginTransaction();
        try{
            $registro = $metasCadastrado->update($request);
            DB::comit();
            return $registro;
        }catch(Exception $e){
            DB::rollback();
    ...
    public function destroy(string $id) {
        $metasCadastrado = $this->show($id);

        DB::beginTransaction();
        try{
            $registro = $metasCadastrado->delete;
            DB::comit();
            return $registro;
        }catch(Exception $e){
            DB::rollback();
